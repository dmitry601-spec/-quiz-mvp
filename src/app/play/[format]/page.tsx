...
export default function PlayPage() {
  ...
  /* track user's answers without causing re-renders */
  ...
  function finishOrAdvance(isCorrect: boolean) {
    ...
    if (index + 1 >= total) {
      sessionStorage.setItem("playclass_answers", JSON.stringify(answersRef.current));
    ...
    function onKey(e: KeyboardEvent) {
      if (question!.format === "quiz") {
        ...
        if (optIdx >= 0 && optIdx < q.options.length) {
          setSelected(optIdx);
          answersRef.current = [...answersRef.current, optIdx];
          const isCorrect = optIdx === q.correct;
          setTimeout(() => {
            const ns = isCorrect ? score + 1 : score;
            if (index + 1 >= total) {
              sessionStorage.setItem("playclass_answers", JSON.stringify(answersRef.current));
              router.push(`/results?score=${ns}&total=${total}&format=${format}`);
        ...
        if (!revealed && (e.key === " " || e.key === "Enter")) {
          ...
          if (e.key === "ArrowLeft") {
            ...
            if (index + 1 >= total) {
              sessionStorage.setItem("playclass_answers", JSON.stringify(answersRef.current));
              router.push(`/results?score=${score}&total=${total}&format=${format}`);
            } else setIndex((i) => i + 1);
          }
          if (e.key === "ArrowRight" || e.key === "Enter") {
            answersRef.current = [...answersRef.current, true];
            const ns = score + 1;
            if (index + 1 >= total) {
              sessionStorage.setItem("playclass_answers", JSON.stringify(answersRef.current));
              router.push(`/results?score=${ns}&total=${total}&format=${format}`);
    ...
    <main style={{
      ...
      <div style={{ width: "100%", maxWidth: "560px", display: "flex", flexDirection: "column", gap: "20px" }}>
        ...
        <div style={{ display: "flex", gap: "6px", justifyContent: "center", alignItems: "center" }} role="progressbar" aria-valuenow={index + 1} aria-valuemin={1} aria-valuemax={total}>
          {Array.from({ length: total }).map((_, i) => (
            <div key={i} style={{
              ...
              borderRadius: "100px",
              background: i <= index ? BLUE : BORDER,
        ...
        <div key={animKey} style={{
          ...
          animation: "fade-up 0.35s ease both", minHeight: "280px",
        }}>
          ...
          return (
            <button key={i} onClick={() => onAnswer(i)} disabled={revealed}
              ...
              style={{
                ...
                padding: "14px 16px", minHeight: "48px", textAlign: "left",
                fontSize: "15px", fontFamily: "'Golos Text', system-ui, sans-serif",
            ...
            >
              <span style={{
                fontSize: "11px", fontWeight: 700, minWidth: "28px", height: "28px",
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: "20px" }}>
      ...
      {!revealed ? (
        <button onClick={onReveal} style={{
          ...
          onMouseEnter={(e) => { const el = e.currentTarget as HTMLElement; el.style.borderColor = BLUE; el.style.color = INK; el.style.background = BLUE_SOFT; }}
          onMouseLeave={(e) => { const el = e.currentTarget as HTMLElement; el.style.borderColor = BORDER; el.style.color = MID; el.style.background = SURFACE; }}
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "14px", animation: "fade-up 0.3s ease both" }}>
          <div style={{ borderRadius: "14px", padding: "18px", background: BLUE_SOFT, border: `1px solid ${BLUE_MID}` }}>
            <span style={{ fontSize: "11px", fontWeight: 700, letterSpacing: "2px", textTransform: "uppercase" as const, color: BLUE }}>Определение</span>
            <p style={{ marginTop: "8px", fontSize: "15px", lineHeight: 1.72, color: INK }}>{question.back}</p>
          </div>
